...
const socket = io();
```
Den här raden skapar en WebSocket-anslutning från klienten till servern med Socket.IO, vilket möjliggör realtidskommunikation mellan klient och server.
***
...
```
Den här koden kontrollerar om kommentarsformuläret finns på sidan och lägger i så fall till en event listener för submit. När formuläret skickas förhindras sidomladdning, kommentaren hämtas och om den inte är tom skickas den vidare för hantering, samtidigt som formulärfältet rensas.
***
...
```
Den här funktionen ansvarar för att skicka en kommentar till servern via WebSocket. Den validerar att kommentaren är en sträng och inte för lång, och skickar den därefter tillsammans med spel-ID:t till servern.
***

### Tar emot kommentarer från server
```js
...
```
Den här koden lyssnar efter nya kommentarer från servern via WebSocket. När en kommentar tas emot läggs den till i kommentarssektionen på sidan som HTML, utan att sidan behöver laddas om.
***
...
```
Den här funktionen påbörjar borttagning av kommentarer på klientsidan. Den visar först en bekräftelseruta för att undvika oavsiktlig radering. Om användaren bekräftar hämtas kommentars-ID:t och skickas till servern via WebSocket för att radera kommentaren från databas.
***
...
```
Den här koden lyssnar efter borttagning av kommentarer från servern via WebSocket. När ett kommentars-ID tas emot tas motsvarande element bort från sidan direkt, utan att sidan behöver laddas om.
***

### Hantering för ändring av kommentarer
```js
...
```
Den här funktionen gör det möjligt att redigera en kommentar direkt på sidan. Den aktiverar redigerbart läge för kommentaren och fokuserar på texten. När användaren trycker på Enter skickas den uppdaterade kommentaren till servern via WebSocket.

Om användaren avbryter redigeringen (tappar fokus) återställs kommentaren till sitt ursprungliga innehåll.
***
...
```
Den här koden lyssnar efter uppdaterade kommentarer från servern via WebSocket. När en ändring tas emot ersätts den gamla kommentarteksten med den nya (escapade) HTML:n direkt på sidan, utan att behöva ladda om.
***

### Dubbelkoll för att radera spel
```js
...
```
Den här koden körs när sidan har laddats klart. Den kontrollerar vilken sida användaren befinner sig på och anpassar visningen av element därefter.

Den kontrollerar också om det finns ett felmeddelande i URL:en och visar i så fall en popup med feltexten till användaren.
***
...
```
Den här funktionen hanterar sökning på startsidan. Den hämtar användarens inmatade söktext och jämför den med titlarna på alla spel. Spel som matchar söktexten visas, medan övriga döljs genom att en CSS-klass läggs till eller tas bort.
***
